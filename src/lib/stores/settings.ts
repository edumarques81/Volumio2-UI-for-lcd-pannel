 ...
 * This preserves user settings when upgrading from volumio-poc to stellar-volumio
 */
function migrateStorageKey(oldKey: string, newKey: string): void {
  if (typeof localStorage !== 'undefined') {
    ...
    if (oldValue !== null && localStorage.getItem(newKey) === null) {
      ...
      console.log(`[Settings] Migrated localStorage: ${oldKey} -> ${newKey}`);
    }
  }
}

// Run migrations on module load (before any stores initialize)
migrateStorageKey('volumio-poc-background', 'stellar-volumio-background');
migrateStorageKey('volumio-poc-lcd-standby-mode', 'stellar-volumio-lcd-standby-mode');
...
const LCD_STANDBY_MODE_KEY = 'stellar-volumio-lcd-standby-mode';
