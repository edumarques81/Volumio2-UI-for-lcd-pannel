<script lang="ts">
  ...
  function handleSortChange(event: Event) {
    ...
    libraryActions.fetchNASAlbums({ sort });
  }
  ...
  onMount(() => {
    initLibraryStore();
    libraryActions.fetchNASAlbums();
  });
</script>

<div class="albums-view" data-view="nasAlbums">
  <!-- Header -->
  <header class="view-header">
    <div class="header-left">
      ...
      <h1 class="title">{$selectedLibraryAlbum ? $selectedLibraryAlbum.title : 'NAS Drives'}</h1>
      {#if $selectedLibraryAlbum}
  ...
  <div class="view-content">
    {#if $libraryAlbumsError}
      <div class="error-message">
        ...
        <button class="retry-btn" on:click={() => libraryActions.fetchNASAlbums()}>
          Retry
        </button>
      </div>
    ...
    {:else if $libraryAlbums.length === 0}
      <div class="empty">
        <Icon name="storage" size={64} />
        <p>No albums found on NAS</p>
        <span class="hint">Connect a NAS drive and add music</span>
      </div>
    {:else}
      <AlbumGrid
